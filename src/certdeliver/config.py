...
class ClientSettings(BaseSettings):
    ...
    server_url: str = Field(
        default="https://localhost:8000/api/v1/",
        description="CertDeliver server URL"
    )
    ...
    cert_name: str = Field(
        default="cert",
        description="Certificate name (matches certbot --cert-name)"
    )
    ...
    post_update_command: Optional[str] = Field(
        default=None,
...
class HookSettings(BaseSettings):
    ...
    cert_name: str = Field(
        default="cert",
        description="Certificate name to package"
    )
